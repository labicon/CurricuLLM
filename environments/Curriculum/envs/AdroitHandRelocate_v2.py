...
class AdroitHandRelocateEnv(MujocoEnv, EzPickle):
    ...
    * v0: legacy versions in the [D4RL](https://github.com/Farama-Foundation/D4RL).
    ...
    def compute_reward_curriculum(self):
        ...
        previous_task_weight = 0.5
        current_task_weight = 1.0
        
        # Calculate differences and their L2 norms for the previous task component
        positional_difference_ball = self.positional_difference_ball()
        positional_difference_ball_norm = np.linalg.norm(positional_difference_ball)
    
        # Calculate differences and their L2 norms for the current task component
        positional_difference_target = self.positional_difference_target()
        positional_difference_target_norm = np.linalg.norm(positional_difference_target)
    
        # Reward components
        reward_components = {
            "previous_task": -previous_task_weight * np.tanh(positional_difference_ball_norm),  # Encourages arm to be close to the ball
            "current_task": -current_task_weight * np.tanh(positional_difference_target_norm)  # Encourages minimizing position difference to target
        }
