...
class MujocoFetchPushEnv(MujocoFetchEnv, EzPickle):
    ...
    * v1: the environment depends on `mujoco_py` which is no longer maintained.
    ...
    def compute_reward_4(self):
        ...
        # Calculate the distance between the block and the goal position
        distance_to_goal = np.linalg.norm(block_pos - goal_pos)
    
        # Define weight for the distance reward
        distance_weight = -1.0  # Negative to make it a cost (we want to minimize this distance)
    
        # Calculate the weighted distance reward
        distance_reward = distance_weight * distance_to_goal
    
        # Total reward is simply the distance reward in this case
        reward = distance_reward
