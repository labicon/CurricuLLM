...
class HalfCheetahEnvFeedback(MujocoEnv, utils.EzPickle):
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, action):
        ...
        reward, reward_dict = compute_reward(observation_before, action, observation_after)
        success = evaluation(observation_before, action, observation_after)
        terminated = False
        info = {
...
def compute_reward(observation: np.array, action: np.array, next_observation: np.array) -> (np.float64, dict):
    BALANCE_THRESHOLD = 0.01
    Fall_THRESHOLD = 0.01
    WEIGHT = 0.1
    
    # Extracting useful observations
    front_tip_angle_current = observation[2]
    front_tip_angle_next = next_observation[2]

    front_tip_z_current = observation[1]
    front_tip_z_next = next_observation[1]
    
    # Reward for maintaining balance (front tip angle should not change much)
    balance_reward = np.exp(-np.abs(front_tip_angle_next - front_tip_angle_current) / BALANCE_THRESHOLD)
    
    # Penalty for falling off (Z coordinate of the front tip should not decrease)
    fall_penalty = 0.0 if (front_tip_z_next < front_tip_z_current) else 1.0
    fall_penalty = fall_penalty / Fall_THRESHOLD
    
    # Total Reward
    reward = balance_reward - WEIGHT * fall_penalty

    return reward, {"balance_reward": balance_reward, "fall_penalty": fall_penalty}

def evaluation(observation: np.array, action: np.array, next_observation: np.array) -> bool:
    BALANCE_THRESHOLD = 0.01
    ...
    # Task is successful if the balance is maintained (front tip angle does not change much)
    success = np.abs(front_tip_angle_next - front_tip_angle_current) <= BALANCE_THRESHOLD
