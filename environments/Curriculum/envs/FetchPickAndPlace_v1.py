...
class MujocoFetchPickAndPlaceEnv(MujocoFetchEnv, EzPickle):
    ...
    * v1: the environment depends on `mujoco_py` which is no longer maintained.
    ...
    def compute_reward_curriculum(self):
        ...
        # Calculate distance between end_effector_position and block_position
        distance_effector_block = np.linalg.norm(end_effector_pos - block_pos)
        # Calculate distance between block_position and goal_position
        distance_block_goal = np.linalg.norm(block_pos - goal_pos)
    
        # Define weight parameters for the reward components
        weight_effector_block = 2.0  # Prioritize moving away after placement
        weight_block_goal = 5.0  # High priority to place the block near the goal
    
        # Create reward components
        # Encourage max distance between effector and block once block is close to goal
        reward_effector_block = np.tanh(1.0 / (distance_effector_block + 0.01)) * (1.0 / (distance_block_goal + 0.01))
        # Penalize distance from block to goal
        reward_block_goal = -distance_block_goal
    
        # Total reward calculation with weights
        total_reward = (weight_effector_block * reward_effector_block) + (weight_block_goal * reward_block_goal)
    
        # Previous tasks' rewards
        # Task: Move to Goal Position
        gripper_distance = self.gripper_distance()
        block_relative_linear_velocity = self.block_relative_linear_velocity()
        
        # Maintain rewards for previous tasks with lesser weights to prevent forgetting
        weight_prev_task_gripper_opening = 0.1
        weight_prev_task_reach_block = 0.1
        weight_prev_task_grasp_block = 0.1
        weight_prev_task_lift_block = 0.1
        weight_prev_task_move_to_goal = 0.2
    
        # Previous tasks' rewards calculations
        reward_gripper_opening = np.tanh(gripper_distance)  # Open Gripper
        distance_to_block = np.linalg.norm(end_effector_pos - block_pos)  # Reach the Block
        reward_distance_to_block = -distance_to_block
        norm_block_relative_linear_velocity = np.linalg.norm(block_relative_linear_velocity)  # Grasp and Lift Block
        reward_block_relative_velocity = -norm_block_relative_linear_velocity
    
        # Sum of weighted rewards for preserving previous learned tasks
        reward_previous_tasks = (weight_prev_task_gripper_opening * reward_gripper_opening + 
                                 weight_prev_task_reach_block * reward_distance_to_block +
                                 weight_prev_task_grasp_block * reward_block_relative_velocity +
                                 weight_prev_task_lift_block * reward_block_relative_velocity +
                                 weight_prev_task_move_to_goal * reward_block_goal)
    
        # Adding the weighted sum of previous tasks' rewards to total reward
        total_reward += reward_previous_tasks
    
        # Reward dictionary
        reward_dict = {
            'distance_effector_block': distance_effector_block,
            'distance_block_goal': distance_block_goal,
            'reward_effector_block': reward_effector_block,
            'reward_block_goal': reward_block_goal,
            'reward_previous_tasks': reward_previous_tasks
        }
