    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.task_list = ["enhanced_stability", "basic_movement", "balanced_control_efficiency", "prioritized_forward_movement"]
        self.task_threshold = {
            "enhanced_stability": 0.7,
            "basic_movement": 0.5,
            "balanced_control_efficiency": 0.6,
            "prioritized_forward_movement": 0.75
        }
        self.task = task
        if self.task is not None and self.task not in self.task_list:
            raise ValueError(f"Task {self.task} is not in the list of tasks: {self.task_list}")
    ...
    def step(self, action):
        ...
        reward = self.subtask_reward(observation_before, action, observation_after)
        terminated = False
        info = {
            ...
            "reward_main": forward_reward - ctrl_cost,
            "task": self.task,
            "reward_task": reward,
        }
    ...
    def subtask_reward(self, observation_before, action, observation_after):
        if self.task == "enhanced_stability":
            return self.reward_enhanced_stability()
        elif self.task == "basic_movement":
            return self.reward_basic_movement(observation_before, action, observation_after)
        elif self.task == "balanced_control_efficiency":
            return self.reward_balanced_control_efficiency(observation_before, action, observation_after)
        elif self.task == "prioritized_forward_movement":
            return self.reward_prioritized_forward_movement(observation_before, action, observation_after)
        elif self.task == None:
            return self.reward_final_environment_goal(observation_before, observation_after, action)

    def reward_enhanced_stability(self):
        observation = self._get_obs()
        z_current = observation[1]
        angular_velocity_front_tip = observation[11]

        z_desired = 0.0
        sigma_z = 0.1
        sigma_omega = 0.1

        r1 = np.exp(- (z_current - z_desired)**2 / (2 * sigma_z**2))
        r2 = np.exp(- angular_velocity_front_tip**2 / (2 * sigma_omega**2))        

        return 0.5*r1 + 0.5*r2
    
    def reward_basic_movement(self, observation_before, action, observation_after):
        x_before = observation_before[0]
        x_after = observation_after[0]
    ...
    def reward_balanced_control_efficiency(self, observation_before, action, observation_after, alpha=0.5):
        x_before = observation_before[0]
        x_after = observation_after[0]
        delta_x = (x_after - x_before) / self.dt
        
        ctrl_cost = 0.1 * np.sum(np.square(action))
        
        return alpha * delta_x - ctrl_cost
    
    def reward_prioritized_forward_movement(self, observation_before, action, observation_after, beta=1.5):
        x_before = observation_before[0]
        x_after = observation_after[0]
        delta_x = (x_after - x_before) / self.dt
        ...
        return beta * delta_x - ctrl_cost
    
    def reward_final_environment_goal(self, observation_before, observation_after, action, forward_reward_weight=1.0):
        x_before = observation_before[0]
        x_after = observation_after[0]
        delta_x = (x_after - x_before) / self.dt
        ...
        return forward_reward_weight * delta_x - ctrl_cost
