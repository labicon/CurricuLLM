    ...
    * v0: Initial versions release (1.0.0).
    ...
    def step(self, action):
        ...
        if self.task is not None:
            reward, reward_dict = compute_reward(observation_before, action, observation, self.task)
        else:
        ...
        info = {
            ...
            "reward_main": forward_reward - ctrl_cost + healthy_reward,
            "reward_task": reward,
...
def compute_reward(observation, action, next_observation, task) -> Tuple[np.float64, Dict[str, np.float64]]:
    if task == "balance":
        reward, reward_dict = compute_reward_balance(observation, action, next_observation)
    elif task == "simple_hop":
        reward, reward_dict = compute_reward_simple_hop(observation, action, next_observation)
    elif task == "forward_movement":
        reward, reward_dict = compute_reward_forward_movement(observation, action, next_observation)
    elif task == "hopping_forward":
        reward, reward_dict = compute_reward_hopping_forward(observation, action, next_observation)
    
    return reward, reward_dict

def compute_reward_balance(observation, action, next_observation) -> Tuple[np.float64, Dict[str, np.float64]]:
...
def compute_reward_simple_hop(observation, action, next_observation) -> Tuple[np.float64, Dict[str, np.float64]]:
...
def compute_reward_forward_movement(observation, action, next_observation) -> Tuple[np.float64, Dict[str, np.float64]]:
...
def compute_reward_hopping_forward(observation, action, next_observation) -> Tuple[np.float64, Dict[str, np.float64]]:
    x_position_before = observation[0]
    x_position_after = next_observation[0]
    z_height_after = next_observation[1]
