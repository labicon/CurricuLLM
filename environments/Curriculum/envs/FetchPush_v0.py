...
class MujocoFetchPushEnv(MujocoFetchEnv, EzPickle):
    ...
    * v1: the environment depends on `mujoco_py` which is no longer maintained.
    ...
    def compute_reward_curriculum(self):
        ...
        # Compute the distance between block and goal
        ...
        approach_weight = 0.1  # Past task, still somewhat important to keep behavior
        velocity_alignment_weight = 0.1  # Past task, less important now but don't want to forget
        initiation_push_weight = 0.1  # Past task, still important for initiating movement
        precise_push_weight = 0.2  # Past task, important as it directly leads to current task
        success_weight = 5.0  # Current task, highest importance
    
        # Retrieve last reward components for continuity
        end_effector_pos = self.end_effector_position()
        rel_velocity = self.block_relative_linear_velocity()
        
        # Re-calculate necessary components for past tasks
        distance_to_block = np.linalg.norm(end_effector_pos - block_pos)  # for approach
        velocity_alignment = np.linalg.norm(rel_velocity)  # for align velocities
        initiation_reward = np.tanh(np.linalg.norm(rel_velocity))  # for initiate push
        z_distance_from_table = np.abs(end_effector_pos[2] - 0.42)  # for precise push towards goal
        
        # Compute reward components
        approach_reward = -approach_weight * distance_to_block
        velocity_alignment_reward = -velocity_alignment_weight * velocity_alignment
        initiation_push_reward = initiation_push_weight * initiation_reward
        precise_push_reward = -precise_push_weight * z_distance_from_table
        success_reward = success_weight * (1 if success else 0)  # Give success reward if successful, 0 otherwise
        
        # Sum up all the rewards
        total_reward = (
            approach_reward + 
            velocity_alignment_reward + 
            initiation_push_reward + 
            precise_push_reward +
            success_reward
        )
        
        # Prepare the reward dictionary for debugging/tracking
        reward_dict = {
            'approach_reward': approach_reward,
            'velocity_alignment_reward': velocity_alignment_reward,
            'initiation_push_reward': initiation_push_reward,
            'precise_push_reward': precise_push_reward,
